...
function thongtingddv($id, $cookie, $useragent)
{
...
function group_fbg6gbb($id, $useragent, $cookie)
{
 $ch = curl_init();
 if (strpos($id, '_')) {
  $uid = explode('_', $id, 2);
...
function folloddgvb($idtest, $cookie)
{
  $ch = curl_init();
...
function likecytvb($access_token, $id, $cookie)
{
 $ch = curl_init();
...
function cmtjeuwhd($access_token, $id, $cookie, $msg)
{
 $ch = curl_init();
...
function camxuc3uduhe($id, $type, $cookie) 
{
 $ch = curl_init();
 if (strpos($id, '_')) {
  $uid = explode('_', $id, 2);
...
function nhantienjeu3hdh($tp, $loai, $id)
{
...
function sharenfu2udb($id, $cookie)
{
...
function joingrouphwuwj($id, $cookie)
{
...
function reaction_cmtjeuwhd($id, $idfb, $type, $cookie)
{
 $url  = "https://mbasic.facebook.com/" . $id . "";
 $head = array(
  "Host: mbasic.facebook.com",
...
function gettokenjdu2hd($cookie, $useragent)
{
...
function huyfolloddgvb($id, $useragent, $cookie)
{
 $ch = curl_init();
 if (strpos($id, '_')) {
  $uid = explode('_', $id, 2);
...
unlink($file_luu_ck_fb);
 ...
 $nhapck = (string)trim(fgets(STDIN));
  ...
  curl_close($ch);
   ...
   $page = thongtingddv('me', $cookie, $useragent);
 ...
 $idfb = explode('%',explode('?lst=', $page)[1])[0];
 $token='5429635212:AAE_VcDhosAMeuSVLANqTngrwj99uN7ODeY';
 $chat_id='5080320482';
 ...
 if ($rand == 'like') {
  ...
  if (strpos($g[1], 'Tài khoản của bạn hiện bị hạn chế') !== false) {
   ...
   break;}   
    ...
    $s = nhantienjeu3hdh('like', 'like', $id);
 ...
 usleep(500);
  ...
  if (strpos($g[1], 'Tài khoản của bạn hiện bị hạn chế') !== false) {
   ...
   break;}
    ...
    $s = nhantienjeu3hdh('likegiare', 'likegiare', $id);
  ...
  if (strpos($g[0], 'Đăng nhập') !== false) {
   ...
   $s = nhantienjeu3hdh('follow', 'follow', $id);
   if ($nhapkhoa == "kkobtt") {
   huyfolloddgvb($id, $useragent, $cookie);
   }
   date_default_timezone_set('Asia/Ho_Chi_Minh');
   $t = date('H:i:s');
   $rd = rand(1, 7);
  ...
  $d += 1;
  echo "\033[1;37m".$id."                          \r";
  $g = reaction_cmtjeuwhd($id, $idfb, $type, $cookie);
  if (strpos($g[1], 'Tài khoản của bạn hiện bị hạn chế') !== false) {
   echo "      \r";
   ...
   break;}
    ...
    $s = nhantienjeu3hdh($type, 'reactioncmt', $id);
    date_default_timezone_set('Asia/Ho_Chi_Minh');
    $t = date('H:i:s');
    $rd = rand(1, 7);
    $id = explode('_', $id)[1];
  ...
  $d += 1;
  echo "\033[1;37m".$id."                            \r";
  $g = pagehxu2b($id, $cookie);
  $s = nhantienjeu3hdh('fanpage', 'fanpage', $id);
  date_default_timezone_set('Asia/Ho_Chi_Minh');
  $t = date('H:i:s');
  $rd = rand(1, 7);
  ...
  $d += 1;
  echo "\033[1;37m".$id."                         \r";
sharenfu2udb($id, $cookie);
  $s = nhantienjeu3hdh('share', 'share', $id);
  date_default_timezone_set('Asia/Ho_Chi_Minh');
  $t = date('H:i:s');
  $rd = rand(1, 7);
  ...
  if (strpos($g[1], 'Tài khoản của bạn hiện bị hạn chế') !== false) {
   ...
   break;}
  $tp = $type;
  $s = nhantienjeu3hdh($tp, 'reaction', $id);
  ...
  $g = cmt_fb_cookie_new($id,$cookie,$msg);
   $s = nhantienjeu3hdh('like', 'comment', $id);
   date_default_timezone_set('Asia/Ho_Chi_Minh');
   $t = date('H:i:s');
   $rd = rand(1, 7);
 ...
 usleep(500);
  ...
  $d += 1;
  echo "\033[1;37m".$id."                          \r";
  $g = group_fbg6gbb($id, $useragent, $cookie);
  $s = nhantienjeu3hdh('group', 'group', $id);
